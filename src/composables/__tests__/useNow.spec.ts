...
describe('useNow', () => {
  it('掛載時先對一次時，之後每 60 秒 tick 一次', () => {
    ...
    expect(ui.now).toBe(new Date('2026-09-21T09:00:00').getTime())
    ...
    expect(ui.now).toBe(new Date('2026-09-21T09:01:00').getTime())
  ...
  it('跨日後 todayIdx / todayIso 跟著走', () => {
    ...
    const before = ui.todayIdx

    vi.advanceTimersByTime(60_000)

    expect(ui.todayIdx).toBe(before + 1)
    expect(ui.todayIso).toBe('2026-09-22')
    wrapper.unmount()
  })
